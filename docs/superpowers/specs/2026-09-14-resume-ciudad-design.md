...
| Plaza y manzanas devoradas | La plaza no es un prisma: sus baldosas son un `ground` de `plaza` a nivel de calle (z 0.05) con `toneOffset` sesgado (la mitad +1, un quinto -1), rodeado por un cordón de cuatro prismas de `paving` de 1.2 de ancho (no de `plaza` de 0.6): así el contorno se lee al zoom de mundo. La torre apoya a z 0. Un `ground` se dibuja antes que los sólidos, así que no puede pintarse sobre el techo de un prisma. Las manzanas devoradas usan el mismo esquema (`ground` de `leafDark` + losas de zócalo sueltas) |
...
| Derrame de luz de la plaza | Los cuatro derrames dejaron de ser `dot` r 3: son dos charcos de tres octógonos `amberBleed` concéntricos (radios 2.5, 1.8, 1.2, z 0.12, alpha 0.25 cada uno), que con poco alfa y superpuestos se deshacen en el borde. Van sobre la franja de baldosas al este de la torre, en la única diagonal (centros (143.5, 222) y (146, 224)) que en pantalla no pisa la pared este, los conos de las esquinas ni los techos de la fila 3: el pie mismo de la pared queda tapado por la propia torre |
| Papeles | Arrancan escalonados: el papel i nace a `(i / n) · PAPER_RANGE` de la ventana, así el frame 0 y reduced-motion muestran un reguero y no un punto. Derivan al **NNE** (`{ x: 0.45, y: -0.893 }`), no al ENE: con la proyección `sx = x - y`, `sy = (x + y)/2 - 1.4 z`, ENE se veía cayendo hacia abajo a la derecha. Cada papel tiene desvío lateral (±1.5, entra gradualmente en 5 u), factor de velocidad 0.8..1.2, gana altura 0.15 u por unidad recorrida y se encoge de r 0.4 a 0.12 en el último tercio antes de reciclarse |
...
| Derrumbe | La rampa al agua va de x 274 a 286 (su labio bajo llega al agua en toda la manzana), la plataforma de `paving` va de 286 a 304, y los tres bloques hundidos están en el agua sin cruzarse: una losa grande de 8×6 (z -0.9, h 1.2) más dos cascotes de 4×3 y 3×4, todos al oeste del labio de la rampa. El cuarto bloque cayó en la calle frente al malecón (`FALLEN_BLOCK` en `city-grid.ts`, única fuente para el derrumbe, los autos y el test) |
| Carriles | Las rayas de carril saltan los cráteres **y la plaza** (el suelo se dibuja en orden de inserción: la calle x 129 le pintaba encima) y no hay cebra bajo el tablero del puente (x 273). `streets()` se partió en `lanes`, `avenue` y `streetFurniture` |
| Skyline | Las manzanas tipo torre sortean `h` 12..18 (no 12..16) y la primera de cada ribera se lleva 17..18, para que el skyline no quede plano abajo del landmark. Desde h 17 el único detalle de techo posible es la sala de máquinas: el tanque (3.1), la antena (4) y los conos de terraza (2.5) pasarían el tope de 21. La segunda mediana este termina en 333.5, al pie del malecón |
